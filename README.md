...
| A shell at launch | Every launch opens one shell in your home directory, before any session. It sits above the session list and has no file; save it (`Alt` + `Shift` + `S` / `Cmd` + `S`) to make it a session, with its programs still running. |
...
| Archiving a session | Drag a row onto the archive dock at the bottom of the sidebar, or right-click → archive. An archived session is out of the list, `Alt` + `1`–`9` and the session ring until you drag or right-click it back, which puts it at the end of the list. The ids are kept in `~/.config/termspace/session-archive.json`. |
