...
| 세션 하나에 YAML 파일 하나 | 세션 파일은 `~/.config/termspace/sessions/`에 둡니다. 앱은 이 파일을 읽기만 하고 알아서 고쳐 쓰지 않습니다. 실행 중에 패인을 나누거나 크기를 바꿔도 파일은 그대로입니다. 세션 목록에서 오른쪽 클릭하면 이름을 바꿀 수 있고, 이때 그 파일의 `name:` 한 줄만 고쳐 씁니다. |
