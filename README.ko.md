...
모든 조작이 키 하나 거리에 있습니다. `Alt` + 방향키로 포커스를 옮기고, `Alt` + `M`으로
캔버스 전체를 보고, `Alt`를 누르고 있으면 화면의 모든 패인에 제목이 뜹니다. 접두 키도,
마우스도 필요 없습니다.
...
| 이름으로 패인 찾아가기 (`Alt` + `K` / `Cmd+Shift+K`) | 패인 제목, 실행 중인 명령, 창 제목 중 몇 글자를 입력하면 해당하는 패인들이 추려집니다. 방향키로 고르고 `Enter`를 누르면 그 패인으로 이동합니다. 글자가 붙어 있지 않아도 됩니다. `nrd`로 `npm run dev`를 찾습니다. 지금 열려 있는 세션 안에서만 찾습니다. |
...
| 켜면 바로 셸 | 앱을 켤 때마다 세션보다 먼저 홈 디렉토리에서 셸 하나가 뜹니다. 세션 목록 위 칸에 따로 표시되고, 파일은 없습니다. 저장하면(`Alt` + `Shift` + `S` / `Cmd` + `S`) 돌던 프로그램을 그대로 둔 채 세션이 됩니다. |
| 세션 목록 순서 바꾸기 | 목록에서 행을 위아래로 끌어서 순서를 정합니다. `Alt`+`1`~`9`도 이 순서대로 세션을 엽니다. 순서는 `~/.config/termspace/session-order.json`에 저장되는데, `sessions/` 폴더가 아니라 설정 파일 옆에 두는 앱 전용 파일입니다. 아직 끌어보지 않은 세션은 파일이 만들어진 순서를 따릅니다. |
...
| 출력 속 링크 | `Ctrl` + 클릭(macOS에서는 `Cmd` + 클릭)으로 URL을 브라우저에서 엽니다. `OSC 8` 하이퍼링크도 똑같이 열립니다. 그냥 클릭하면 지금처럼 선택만 됩니다. |
